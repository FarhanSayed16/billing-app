...
class _InvoiceDetailScreenState extends ConsumerState<InvoiceDetailScreen> {
  ...
  Future<void> _reShareInvoice() async {
    ...
    try {
      ...
      // Ensure the items match what PDF generator expects
      final List<dynamic> formattedItems = (inv['items'] as List).map((i) => {
        'name': i['name'],
        'quantity': i['quantity'],
        'unit_price': double.tryParse(i['unit_price'].toString()) ?? 0,
      }).toList();
      ...
      final pdfFile = await InvoicePdfGenerator.generateInvoicePdf(
        formattedInvoiceData, 
        Map<String, dynamic>.from(inv['store'] ?? {})
      );
      ...
      if (mounted) {
        await showModalBottomSheet(
          ...
          builder: (_) => ShareInvoiceSheet(
            ...
            storeName: inv['store']?['name'] ?? 'Store',
            grandTotal: double.tryParse(inv['grand_total'].toString()) ?? 0,
            billingId: inv['billing_id'] ?? '',
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Failed to load share sheet'), backgroundColor: AppTheme.errorColor));
      }
    } finally {
      if (mounted) setState(() => _isVoiding = false);
    }
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              if ((double.tryParse(inv['discount_amount'].toString()) ?? 0) > 0)
                _TotalRow(label: 'Discount', value: '-${inv['discount_amount']}'),
              if ((double.tryParse(inv['loyalty_discount'].toString()) ?? 0) > 0)
